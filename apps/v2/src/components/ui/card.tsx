...
function getBadgePosition(
	index: number,
	totalNumberOfBadges: number,
): VariantProps<typeof badgeVariants>["position"] {
	switch (true) {
		case totalNumberOfBadges === 1 ||
			(totalNumberOfBadges === 3 && index === 1):
			return "center";
		...
		case totalNumberOfBadges === 4 && index === 0:
			return "even-1";
		case (totalNumberOfBadges === 2 && index === 0) ||
			(totalNumberOfBadges === 4 && index === 1):
			return "even-2";
		case (totalNumberOfBadges === 2 && index === 1) ||
			(totalNumberOfBadges === 4 && index === 2):
			return "even-3";
	...
	return (
		<div className={cn(cardWrapperVariants({ variant }))}>
			<div className={cn(cardContainerVariants({ variant }))}>
				<div
					className="relative h-[104%] col-start-2 translate-y-5 shadow-inner before:svg-portrait"
					...
					{badges.map((badge, i) => (
						<span
							...
							className={badgeVariants({
								position: getBadgePosition(i, badges.length),
							})}
						>
